...
public class Level3Screen implements Screen {
    ...
    public Level3Screen(final MainGame game) {
        ...
        game.playLevelMusic();
        logic = new GameLogic(level3Grid);
    ...
    private void createUI() {
        ...
        nextButton.addListener(new ClickListener() {
            @Override
            public void enter(InputEvent event, float x, float y, int pointer, com.badlogic.gdx.scenes.scene2d.Actor fromActor) {
                int maxUnlocked = progressPrefs.getInteger("maxUnlockedLevel", 3);
                if (maxUnlocked >= 4) {
    ...
    public void render(float delta) {
        ...
        if (logic.isWin() && !isWin) {
            isWin = true;
            int prev = progressPrefs.getInteger("maxUnlockedLevel", 3);
            if (prev < 4) {
                progressPrefs.putInteger("maxUnlockedLevel", 4);
                progressPrefs.flush();
            }
            showCongratulations();
